...
public protocol PopupPresenting: UIViewController {
...
public extension PopupPresenting {
  ...
  func makeConstraints() {
    ...
    case let .center(width, height):
      switch width {
      case let .fixed(size):
        view.autoSetDimension(.width, toSize: size)

      case let .ratio(ratio):
        view.autoMatch(.width, to: .width, of: view.superview!, withMultiplier: ratio)

      case let .pinEdge(insets):
        view.autoPinEdge(toSuperviewEdge: .leading, withInset: insets)
        view.autoPinEdge(toSuperviewEdge: .trailing, withInset: insets)
      ...
      switch height {
      case let .fixed(size):
        view.autoSetDimension(.height, toSize: size)

      case let .ratio(ratio):
        view.autoMatch(.height, to: .height, of: view.superview!, withMultiplier: ratio)

      case let .pinEdge(insets):
        view.autoPinEdge(toSuperviewEdge: .top, withInset: insets)
        view.autoPinEdge(toSuperviewEdge: .bottom, withInset: insets)
